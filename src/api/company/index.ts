...
import { CompanyStateSet, CompanyType, Query, Records } from "@kysion/types";
import { My } from "./my";
...
export class Company {
    ...
    public fetchCompanyList(params: Query) {
        return http.post<Records<CompanyType>>(`/${this.urlPrefix}/queryCompanyList`, params);
    }

    /**
     * 创建企业
     */
    public createCompany(data: Partial<CompanyType> & { name: string; contactName: string }) {
        return http.post<CompanyType>(`/${this.urlPrefix}/createCompany`, data);
    }

    /**
     * 更新企业信息
     */
    public upgradeCompany(data: Partial<CompanyType> & { name: string; contactName: string }) {
        return http.post<CompanyType>(`/${this.urlPrefix}/updateCompany`, data);
    }
    ...
    public getCompanyById(id: string | number) {
        return http.post<CompanyType>(`/${this.urlPrefix}/getCompanyById`, { id });
    }
    ...
    public getCompanyDetail(id: string | number) {
        return http.post<CompanyType>(`/${this.urlPrefix}/getCompanyDetail`, { id, include: ['*'] });
    }
    ...
    public hasCompanyByName(name: string) {
        return http.post<CompanyType>(`/${this.urlPrefix}/hasCompanyByName`, { name });
    }
